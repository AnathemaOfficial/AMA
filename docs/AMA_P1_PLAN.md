...
## Multi-Agent Hardening Plan for AMA
...
AMA P0 proved that the architecture works in real-world actuation:
...
P1 is about making AMA **locally correct under concurrency**.

The objective of P1 is to transform AMA from a **single-agent / controlled-context prototype** into a **multi-agent-safe local membrane**.
...
AMA P1 covers only local runtime hardening:
...
AMA does **not** reason.
AMA does **not** interpret intent semantically.
AMA does **not** plan.

AMA only decides whether an admissible action can cross the membrane.
...
- If key is `ABSENT`, AMA may reserve it and transition to `IN_FLIGHT`.
- If key is already `IN_FLIGHT`, AMA must not start a second execution.
- If key is `DONE`, AMA returns the committed result.
...
6. **Multi-agent local test**
   - at least two concurrent adapters can use AMA without breaking invariants
...
- `docs/AMA_P1_PLAN.md` (this document)
- `docs/AMA_CONCURRENCY_MODEL.md`
- `docs/AMA_IDEMPOTENCY_STATE_MACHINE.md`
- `docs/AMA_QUEUE_MODEL.md`
- `docs/AMA_TIMEOUTS.md`
...
AMA remains a **law-layer membrane for actuation**.

---

## 12. P1.5 Preview — Capability Model Canonicalization

After P1, AMA should evolve from loose "intent" declarations toward explicit **capability manifests**.
...
Only after P1 is held locally, and after the capability model is stabilized, should AMA expand toward:
...
AMA P1 is the phase where AMA stops being merely a successful prototype and becomes a locally reliable law-layer for multi-agent actuation.
...
- Claude Code deployed AMA on Little-Terminator (Linux, RTX 5070 Ti)
- CLI-Anything was installed and used to generate Blender CLI
- First real GPU render was executed through AMA's `proc.exec.bounded` domain
- The full pipeline was demonstrated: Agent → AMA → CLI-Anything → Blender → GPU render in ~4s

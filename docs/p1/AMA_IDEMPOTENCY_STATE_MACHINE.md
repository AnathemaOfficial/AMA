...
This document defines the canonical idempotency model for AMA.

The purpose of idempotency in AMA is not merely to cache prior results.
...
In AMA, idempotency is part of the admission law.
...
idempotency key within a single AMA node.
...
AMA must guarantee the following invariant:
...
The key has no active or committed request record on the local AMA node.
...
AMA must not start a new execution.
...
- Consistent with AMA's philosophy: deterministic, minimal, non-blocking

The only forbidden behavior is duplicate execution.

### 7.3 Replay after DONE

If a request arrives with a key already in `DONE`, AMA must:
...
The same key on the same AMA node must follow a deterministic lifecycle.
...
In AMA, idempotency is not an optimization.
...
That is why AMA idempotency must be implemented as a state machine.

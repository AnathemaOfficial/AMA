...
This document defines SAFA's bounded admission model under concurrent
...
SAFA uses tower's `concurrency_limit(8)` layer as its bounded admission
...
- Queue is implicit (tower layer), not a first-class SAFA construct
